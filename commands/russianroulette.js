const { EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, SlashCommandBuilder } = require('discord.js');
const db = require('../database');

const activeGames = new Map();

module.exports = {
  data: new SlashCommandBuilder()
    .setName('russianroulette')
    .setDescription('Spiele Russisches Roulette mit bis zu 6 Spielern')
    .addStringOption(opt => opt.setName('betrag').setDescription('Einsatz').setRequired(true)),
  async execute(interaction) {
    const userId = interaction.user.id;
    const config = require('../config.json');

    if (activeGames.has(interaction.channel.id)) {
      const game = activeGames.get(interaction.channel.id);
      if (!game.started && !game.players.has(userId)) {
        const balance = db.getBalance(userId);
        if (balance < game.bet) {
          return await interaction.reply(`❌ Du brauchst **${config.currencySymbol}${game.bet}** zum Mitspielen!`);
        }
        game.players.set(userId, interaction.user.username);
        return await interaction.reply(`🔫 **${interaction.user.username}** tritt bei! (${game.players.size}/6 Spieler)`);
      }
      return await interaction.reply('❌ Es läuft bereits ein Spiel in diesem Channel!');
    }

    const amountStr = interaction.options.getString('betrag');
    if (!amountStr) return await interaction.reply('❌ Nutzung: `/russianroulette <Betrag>`');

    const bet = parseInt(amountStr);
    if (!bet || bet <= 0) return await interaction.reply('❌ Ungültiger Betrag!');
    if (bet > db.getBalance(userId)) return await interaction.reply(`❌ Du hast nur **${config.currencySymbol}${db.getBalance(userId)}**!`);

    const game = {
      bet,
      players: new Map([[userId, interaction.user.username]]),
      started: false,
    };
    activeGames.set(interaction.channel.id, game);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`rr_join_${interaction.channel.id}`).setLabel('Beitreten').setStyle(ButtonStyle.Primary).setEmoji('🔫'),
      new ButtonBuilder().setCustomId(`rr_start_${interaction.channel.id}`).setLabel('Starten (2+ Spieler)').setStyle(ButtonStyle.Success).setEmoji('💀'),
    );

    const embed = new EmbedBuilder()
      .setColor('#e74c3c')
      .setTitle('🔫 Russisches Roulette')
      .setDescription(
        `**${interaction.user.username}** startet eine Runde!\n\n` +
        `Einsatz: **${config.currencySymbol}${bet}** pro Spieler\n` +
        `Spieler: **1/6**\n\n` +
        `Tritt bei oder starte das Spiel!`
      )
      .setFooter({ text: '60 Sekunden zum Beitreten' })
      .setTimestamp();

    const msg = await interaction.reply({ embeds: [embed], components: [row], fetchReply: true });
    const collector = msg.createMessageComponentCollector({ time: 60000 });

    collector.on('collect', (btnInteraction) => {
      const game = activeGames.get(interaction.channel.id);
      if (!game) return;

      if (btnInteraction.customId === `rr_join_${interaction.channel.id}`) {
        if (game.players.has(btnInteraction.user.id)) {
          return btnInteraction.reply({ content: '❌ Du bist schon dabei!', flags: 64 });
        }
        if (game.players.size >= 6) {
          return btnInteraction.reply({ content: '❌ Spiel ist voll (6/6)!', flags: 64 });
        }
        const balance = db.getBalance(btnInteraction.user.id);
        if (balance < game.bet) {
          return btnInteraction.reply({ content: `❌ Du brauchst ${config.currencySymbol}${game.bet}!`, flags: 64 });
        }
        game.players.set(btnInteraction.user.id, btnInteraction.user.username);

        const embed = new EmbedBuilder()
          .setColor('#e74c3c')
          .setTitle('🔫 Russisches Roulette')
          .setDescription(
            `Einsatz: **${config.currencySymbol}${game.bet}** pro Spieler\n` +
            `Spieler: **${game.players.size}/6**\n\n` +
            [...game.players.values()].map(n => `🔹 ${n}`).join('\n')
          )
          .setTimestamp();
        btnInteraction.update({ embeds: [embed], components: [row] });

      } else if (btnInteraction.customId === `rr_start_${interaction.channel.id}`) {
        if (btnInteraction.user.id !== userId) {
          return btnInteraction.reply({ content: '❌ Nur der Ersteller kann starten!', flags: 64 });
        }
        if (game.players.size < 2) {
          return btnInteraction.reply({ content: '❌ Mindestens 2 Spieler nötig!', flags: 64 });
        }

        collector.stop('started');
        game.started = true;

        const playerIds = [...game.players.keys()];
        for (const pid of playerIds) {
          if (db.getBalance(pid) < game.bet) {
            activeGames.delete(interaction.channel.id);
            return btnInteraction.update({ content: '❌ Ein Spieler hat nicht mehr genug Geld!', embeds: [], components: [] });
          }
        }

        const loserIdx = Math.floor(Math.random() * playerIds.length);
        const loserId = playerIds[loserIdx];
        const loserName = game.players.get(loserId);

        const totalPot = game.bet * playerIds.length;
        const winShare = Math.floor(totalPot / (playerIds.length - 1));

        for (const pid of playerIds) {
          if (pid === loserId) {
            db.updateBalance(pid, -game.bet);
          } else {
            db.updateBalance(pid, winShare - game.bet);
          }
        }

        const chambers = playerIds.map((pid, i) => {
          if (i < loserIdx) return `🔹 ${game.players.get(pid)} — *klick* ... überlebt!`;
          if (i === loserIdx) return `💀 **${game.players.get(pid)}** — 💥 BANG!`;
          return `⬜ ${game.players.get(pid)}`;
        });

        const embed = new EmbedBuilder()
          .setColor('#e74c3c')
          .setTitle('🔫 Russisches Roulette — Ergebnis')
          .setDescription(
            chambers.join('\n') +
            `\n\n💀 **${loserName}** hat verloren und **${config.currencySymbol}${game.bet}** bezahlt!\n` +
            `Die anderen teilen sich den Pot (**${config.currencySymbol}${winShare}** je Spieler).`
          )
          .setTimestamp();

        activeGames.delete(interaction.channel.id);
        btnInteraction.update({ embeds: [embed], components: [] });
      }
    });

    collector.on('end', (_, reason) => {
      if (reason !== 'started') {
        activeGames.delete(interaction.channel.id);
        msg.edit({ components: [] });
      }
    });
  },
};
