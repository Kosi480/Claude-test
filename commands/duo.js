...
module.exports = {
  name: 'duo',
  aliases: ['partner', 'teamraub', 'coop'],
  description: 'Kooperativer Raub zu zweit! (!duo @partner, 5min CD)',
  execute(message, args) {
    const userId = message.author.id;
    const config = require('../config.json');
    const target = message.mentions.users.first();

    if (!target) return message.reply('❌ Erwähne einen Partner! `!duo @partner`');
    if (target.id === userId) return message.reply('❌ Du brauchst einen echten Partner!');
    if (target.bot) return message.reply('❌ Bots können nicht mitmachen!');
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${message.author.username}** sucht einen Partner für einen Raub!\n\n` +
    ...
    message.reply({ embeds: [embed], components: [row] }).then(msg => {
      ...
      collector.on('collect', (interaction) => {
        if (interaction.user.id !== target.id) {
          return interaction.reply({ content: '❌ Nur die eingeladene Person kann reagieren!', flags: 64 });
        }

        collector.stop();

        if (interaction.customId === `duo_decline_${userId}`) {
          const embed = new EmbedBuilder()
            .setColor('#95a5a6')
            .setTitle('🤝 Abgelehnt')
        ...
        if (success) {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `**Rollen:**\n` +
              `${role1.emoji} **${message.author.username}** — ${role1.name}\n` +
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              ...
              `${role2.emoji} **${target.username}** — ${role2.name}\n\n` +
              `Ihr wurdet erwischt! 🚨\n` +
              `Strafe: **${message.author.username}** -${config.currencySymbol}${fine1} | **${target.username}** -${config.currencySymbol}${fine2}`
            )
            .setFooter({ text: 'Erfolgsrate: ' + Math.floor(successChance * 100) + '%' })
            .setTimestamp();
